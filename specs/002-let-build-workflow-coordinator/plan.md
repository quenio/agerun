...
steps: first confirm that the active workflow definition is valid and that any required local
`complete(...)` dependency is available, then seed exactly one workflow item and drive it through a
single-pass lifecycle. The active workflow definition still lives in a workflow definition file read through the
file delegate rather than in a `.method` file, but first-release workflow definition content is intentionally shaped for
reliable extraction by AgeRun methods. The reusable workflow-definition method owns both workflow definition-backed
...
methods/agents, workflow-definition data read from workflow files via the file delegate, and local
...
behavior lives in AgeRun methods; launch from `bootstrap`; workflow definition stays in workflow definition files, not
`.method`; bundled default validation depends on local `complete(...)`; duplicate boot triggers in
one runtime must be ignored after the first accepted launch; invalid workflow definitions and reporting
...
instructions; no map literals; no nested function calls; keep workflow-definition parse-friendly for method-level
...
demo; one bundled default workflow definition file plus one alternate workflow definition file for tests;
...
- [x] TDD approach defined: add failing method tests for workflow-definition startup probing,
  schema rejection, `complete(...)` decision extraction, retryable `stay` handling, visible
...
- [x] Convention impact reviewed: new method assets stay in `methods/` with
  `<name>-<version>.method` plus matching docs/tests, workflow workflow files stay separate from
...
- [x] Specification and documentation impact identified: `specs/002-let-build-workflow-coordinator/*`,
  ...
  workflow definition-file documentation, `CHANGELOG.md`, and `.specify/memory/pi-agent.md`; `SPEC.md`
...
├── workflow-definition-1.0.0.method       # new workflow definition resolver / `complete(...)` evaluator
├── workflow-definition-1.0.0.md           # new
└── workflow_definition_tests.c            # new

workflows/
├── default.workflow                  # bundled default workflow definition with `complete(...)`
└── test.workflow                     # alternate workflow definition for deterministic validation
...
workflow-definition agent both reads the workflow definition schema and owns `complete(...)`-driven transition
logic. The workflow definition contract is first-release parse-friendly rather than fully general, so methods can
