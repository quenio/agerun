...
workflow summary: item=demo-001 outcome=completed stage=review status=approved owner=ops transitions=4 reason="review confirmed completion criteria"
```
...
- workflow-visible progress and final summaries surface the relevant `reason` generated by
  transition validation when one exists
