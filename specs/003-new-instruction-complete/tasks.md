...
- [x] T006 [P] Add failing specialized parser tests in new `modules/ar_complete_instruction_parser_tests.c` for one-argument parsing, two-argument parsing, evaluation-time rejection of non-map second arguments, rejection of placeholder names that do not follow the existing `build(...)`/`parse(...)` identifier syntax, and rejection of unsupported interpolation syntax beyond `{name}`
- [x] T007 [P] Add failing specialized evaluator tests in new `modules/ar_complete_instruction_evaluator_tests.c` for returned-map success, one-argument versus values-map input, input-map preservation, rejection of leading/trailing whitespace, and rejection of generated `{` or `}` characters
...
more placeholders and have AgeRun return a new map with non-empty generated string values for
missing placeholders while preserving any provided values map.

**Independent Test**: Execute `memory.result := complete("The largest country in South America is {country}.")` and
`memory.result := complete("The capital of {country} is {city}.", memory.values)` through the instruction runtime and verify
that the returned map contains `country` or `city` text generated by the chosen local language
model, that generated values contain no leading/trailing whitespace and no `{` or `}` characters,
that caller-provided values are copied without mutation, and that existing provided values are
...
- [x] T015 [US1] Implement one-argument and two-argument `complete(...)` parsing plus facade dispatch wiring in new `modules/ar_complete_instruction_parser.zig`, `modules/ar_instruction_parser.c`, `modules/ar_instruction_parser.md`, `modules/ar_instruction_parser_tests.c`, and `modules/ar_complete_instruction_parser_tests.c`, accepting any expression as the optional second argument, enforcing at evaluation time that it produces a map, ensuring placeholder names follow the same identifier syntax used by `build(...)` and `parse(...)`, and rejecting unsupported interpolation syntaxes beyond `{name}`
...
- [x] T017 [US1] Implement values-map evaluation, string-only generated-value staging, returned-map result handling, model-generated value acceptance, input-map preservation, rejection of leading/trailing whitespace, and rejection of generated `{` or `}` characters in new `modules/ar_complete_instruction_evaluator.zig`, new `modules/ar_complete_instruction_evaluator_tests.c`, `modules/ar_instruction_evaluator.zig`, `modules/ar_instruction_evaluator.md`, and `modules/ar_instruction_evaluator_tests.c`
...
- [x] T020 [US1] Refactor shared template, placeholder, and result-map helpers in `modules/ar_complete_instruction_parser.zig`, `modules/ar_complete_instruction_evaluator.zig`, and `modules/ar_local_completion.c` while preserving green User Story 1 tests

**Checkpoint**: `complete(...)` can parse, dispatch, return a result map, preserve provided values, and generate missing placeholders for the core happy path.

---

## Phase 4: User Story 2 - Reuse returned variables in later method logic (Priority: P2)

**Goal**: Ensure returned completion-map values behave like normal AgeRun data so later assignments,
...
the returned strings are reusable without conversion.
...
**Checkpoint**: Returned completion values behave like normal string data and can be reused directly in later AgeRun logic.

---

## Phase 5: User Story 3 - Fail safely when completion is unavailable (Priority: P3)

**Goal**: Ensure unavailable local resources, invalid second arguments, invalid placeholder syntax,
incomplete placeholder coverage, empty generated values, generated brace markers, and timeout
conditions fail safely with actionable errors and no provided-map mutation.

**Independent Test**: Execute failing `complete(...)` scenarios through the instruction runtime and
verify that the assigned result is an empty map, actionable errors are recorded, provided input maps
remain unchanged, empty generated values are rejected, and later non-completion work still succeeds.

### Validation for User Story 3

- [x] T028 [P] [US3] Add or update failing failure-path tests in `modules/ar_local_completion_tests.c`, `modules/ar_complete_instruction_evaluator_tests.c`, and `modules/ar_instruction_evaluator_tests.c` for non-map second arguments, invalid-before-generation rejection without local completion invocation, placeholder names that violate the supported identifier syntax, unsupported interpolation syntax beyond `{name}`, missing or unusable local completion resources on otherwise supported environments, environment-specific resource behavior differences that prevent completion, incomplete placeholder coverage, empty generated values, leading/trailing whitespace, generated `{` or `}` characters, partial-generation failure, and timeout behavior
...
- [x] T031 [US3] Implement evaluator-side non-map second-argument rejection, invalid-placeholder-name rejection, incomplete-placeholder and partial-generation failure handling, empty-string rejection, leading/trailing-whitespace rejection, generated-brace rejection, actionable-error recording with failure category, immediate cause, and recovery hint, timeout, next-queued-message blocking semantics, immediate post-failure scheduling readiness, and no-provided-map-mutation behavior in `modules/ar_complete_instruction_evaluator.zig`, `modules/ar_complete_instruction_evaluator.md`, `modules/ar_complete_instruction_evaluator_tests.c`, `modules/ar_instruction_evaluator.zig`, and `modules/ar_instruction_evaluator_tests.c`
...
- [x] T040 Execute the documented 20-template warm-run and cold-start validation procedure in a Linux containerized environment using the fixture set, repeated-placeholder latency treatment, invalid-before-generation fast-failure behavior, next-queued-message timing boundary, single-active-`complete(...)` assumption, below-baseline/heavier-runtime no-guarantee rule, and the documented Linux containerized baseline from `specs/003-new-instruction-complete/quickstart.md`, then record the baseline, timing method, and results in `specs/003-new-instruction-complete/contracts/local-completion-runtime.md` and `specs/003-new-instruction-complete/quickstart.md`
...
3. Deliver User Story 1 (`complete(...)` parsing, local runtime happy path, AST/evaluator dispatch, returned-map behavior)
