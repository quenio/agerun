...
- [x] CHK002 Are requirements defined for all declared output surfaces: returned completion map and generated placeholder values? [Completeness, Spec §Functional Requirements FR-004/FR-008]
- [x] CHK003 Are invalid-input requirements complete for non-map second arguments and invalid placeholder syntax? [Completeness, Spec §Functional Requirements FR-014/FR-014a]
...
- [x] CHK009 Is the meaning of complete result-map creation defined clearly enough for reviewers to distinguish it from partial generated output? [Clarity, Spec §Functional Requirements FR-015b]
...
- [x] CHK012 Are the rules for one-argument vs values-map calls consistent across clarifications, assumptions, and functional requirements? [Consistency, Spec §Clarifications; FR-004a/FR-004b; Assumptions]
- [x] CHK013 Are provided-value-preservation and no-partial-output-on-failure requirements consistent with each other and free of contradictory wording? [Consistency, Spec §Functional Requirements FR-015a/FR-015b/FR-016]
...
- [x] CHK018 Are primary-flow requirements defined for no-values calls, values-map calls, repeated placeholders, and downstream reuse? [Coverage, Spec §User Story 1; User Story 2]
- [x] CHK019 Are exception-flow requirements defined for unavailable local resources, invalid values-map arguments, invalid placeholder syntax, and timeout conditions? [Coverage, Spec §User Story 3; FR-011; FR-014; FR-014a; FR-018]
...
- [x] CHK030 Is it unambiguous whether the instruction may be used without assigning its returned map? [Ambiguity, Spec §FR-008]
