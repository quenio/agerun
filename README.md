...
- **Published decks**:
  ...
  - [Workflow Methodology](https://quenio.github.io/agerun/walkthroughs/workflow-methodology/index.html)
...
- List processing: `append(memory.items, value)` mutates an existing memory-owned list, while `head(list)` and `tail(list)` return shallow-copied list parts for recursive self-message processing without loops or indexed access
