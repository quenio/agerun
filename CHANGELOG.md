...
- **Fixed Linux-only CI failures exposed after complete-runtime-ready succeeded**
  ...
  **Implementation**: Removed the redundant NULL check from the dlsym `strdup` interceptor because
  ...
  skipped the vocab-only direct-backend failure subtest when sanitizer builds or missing CI fixtures
  make it inapplicable while keeping fake-runner and failure-path coverage active; and revised
  ...
  **Verification**: `gh run view 24922732835 --log-failed` and downloaded build logs identified the
  ...
  flags, `gh run view 24923836209 --log-failed` plus downloaded build logs for the missing vocab-only
  fixture, `make clean build 2>&1`, and `make check-logs`.
