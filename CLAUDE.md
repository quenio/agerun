...
3. **Memory issues**:
   ...
   - **Zero memory leaks standard** - the project has achieved zero memory leaks across all modules:
     ...
     - **MANDATORY: Always check the `bin/heap_memory_report.log` file after EACH test run**
     - **CRITICAL: The report file is overwritten on each program run - check it immediately after each test**
     ...
     - **Workflow: Run test → Check heap_memory_report.log → Run next test → Check heap_memory_report.log**
