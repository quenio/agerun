...
- **Methods**: All method definitions are persisted in a file named `agrun.methods`. They are loaded and made available when the agent system restarts.
- **Agents**: Agents and their memory are not persisted by default. However, if an agent is created with the `persist: boolean` option set to `true` (default is `false`), its memory dictionary and context are saved to a file named `agrun.agents`. The persisted agents are automatically restored at system startup before the first agent is executed.
