...
/**
 * Log an error message with position.
 */
static void _log_error(ar_build_instruction_parser_t *mut_parser, const char *ref_message, size_t position) {
 ...
 */
static ar_list_t* _parse_arguments_to_asts(ar_build_instruction_parser_t *mut_parser, 
                                        char **ref_args, 
                                        size_t arg_count,
                                        size_t error_offset) {
...
ar_instruction_ast_t* ar_build_instruction_parser__parse(
    ...
    if (strncmp(ref_instruction + pos, "build", 5) != 0) {
        _log_error(mut_parser, "Expected 'build' function", pos);
        return NULL;
    }
    pos += 5;
    ...
    if (ref_instruction[pos] != '(') {
        _log_error(mut_parser, "Expected '(' after 'build'", pos);
        return NULL;
    }
    pos++;
    ...
    ar_instruction_ast_t *own_ast = ar_instruction_ast__create_function_call(
        AR_INSTRUCTION_AST_TYPE__BUILD, "build", const_args, arg_count, ref_result_path
    );
