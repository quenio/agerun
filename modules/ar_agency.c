...
bool ar_agency__agent_has_messages(int64_t agent_id) {
    ...
    agent_t *ref_agent = (agent_t*)ar_agent_registry__find_agent(g_own_registry, agent_id);
    if (!ref_agent) {
        return false;
    }
...
ar_data_t* ar_agency__get_agent_message(int64_t agent_id) {
    ...
    agent_t *mut_agent = (agent_t*)ar_agent_registry__find_agent(g_own_registry, agent_id);
    if (!mut_agent) {
        return NULL;
    }
...
int64_t ar_agency__create_agent(const char *ref_method_name, const char *ref_version, const ar_data_t *ref_context) {
    ...
    agent_t *own_agent = ar_agent__create(ref_method_name, ref_version, ref_context);
    if (!own_agent) {
        return 0;
    }
...
bool ar_agency__destroy_agent(int64_t agent_id) {
    ...
    agent_t *own_agent = (agent_t*)ar_agent_registry__find_agent(g_own_registry, agent_id);
    if (!own_agent) {
        return false;
    }
...
bool ar_agency__set_agent_id(int64_t old_id, int64_t new_id) {
    ...
    agent_t *mut_agent = (agent_t*)ar_agent_registry__find_agent(g_own_registry, old_id);
    if (!mut_agent) {
        return false;
    }
