...
static void test_create_parser_with_log(void) {
    ...
    ar_destroy_instruction_parser_t *parser = ar_destroy_instruction_parser__create(log);
...
static void test_destroy_agent_parser__create_destroy(void) {
    ...
    ar_destroy_instruction_parser_t *own_parser = ar_destroy_instruction_parser__create(NULL);
    
    // Then it should be created successfully
    assert(own_parser != NULL);
    
    // When destroying the parser
    ar_destroy_instruction_parser__destroy(own_parser);
...
static void test_destroy_agent_parser__parse_integer_id(void) {
    ...
    ar_destroy_instruction_parser_t *own_parser = ar_destroy_instruction_parser__create(log);
    assert(own_parser != NULL);
    
    ar_instruction_ast_t *own_ast = ar_destroy_instruction_parser__parse(own_parser, instruction, NULL);
...
static void test_destroy_agent_parser__parse_with_assignment(void) {
    ...
    ar_instruction_ast_t *own_ast = ar_destroy_instruction_parser__parse(own_parser, instruction, "memory.result");
...
static void test_destroy_agent_parser__error_handling(void) {
    ...
    ar_instruction_ast_t *ast = ar_destroy_instruction_parser__parse(own_parser, "destroy 123", NULL);
    assert(ast == NULL);
    assert(ar_log__get_last_error_message(log) != NULL);
    assert(strstr(ar_log__get_last_error_message(log), "Expected '(' after 'destroy'") != NULL);
    
    // Test 2: Wrong function name
    ast = ar_destroy_instruction_parser__parse(own_parser, "delete(123)", NULL);
    assert(ast == NULL);
    assert(strstr(ar_log__get_last_error_message(log), "Expected 'destroy' function") != NULL);
    
    // Test 3: Empty arguments
    ast = ar_destroy_instruction_parser__parse(own_parser, "destroy()", NULL);
    assert(ast == NULL);
    ...
    ast = ar_destroy_instruction_parser__parse(own_parser, "destroy(123, 456)", NULL);
    assert(ast == NULL);
    assert(ar_log__get_last_error_message(log) != NULL);
...
static void test_destroy_agent_parser__parse_with_expression_asts(void) {
    ...
    ar_instruction_ast_t *own_ast = ar_destroy_instruction_parser__parse(own_parser, instruction, "memory.destroyed");
    ...
    ar_instruction_ast_t *own_ast2 = ar_destroy_instruction_parser__parse(own_parser, instruction2, NULL);
    
    assert(own_ast2 != NULL);
    assert(ar_instruction_ast__get_type(own_ast2) == AR_INSTRUCTION_AST_TYPE__DESTROY);
