    ...
    if (own_ctx->own_results) {
        ...
        if (own_items && count > 0) {
            ...
            for (size_t i = 0; i < count; i++) {
                ar_data_t *ref_result = (ar_data_t *)own_items[i];
                if (ref_result) {
...
static char* parse_identifier(ar_expression_context_t *mut_ctx) {
    ...
    char *own_identifier = AR__HEAP__MALLOC((size_t)length + 1, "Expression identifier");
    if (!own_identifier) {
        return NULL;
    }
...
static const ar_data_t* parse_string_literal(ar_expression_context_t *mut_ctx) {
    ...
    char *own_temp_str = AR__HEAP__MALLOC((size_t)len + 1, "Temporary string buffer");
    if (!own_temp_str) {
        return NULL;
    }
    ...
    ar_data_t *own_result = ar_data__create_string(own_temp_str);
...
static const ar_data_t* parse_number_literal(ar_expression_context_t *mut_ctx) {
    ...
    if (mut_ctx->ref_expr[mut_ctx->offset] == '.') {
        ...
        ar_data_t *own_result = ar_data__create_double(double_value);
    ...
    ar_data_t *own_result = ar_data__create_integer(value);
...
static const ar_data_t* parse_multiplicative(ar_expression_context_t *ctx) {
    ...
    while (ctx->ref_expr[ctx->offset] == '*' || ctx->ref_expr[ctx->offset] == '/') {
        ...
        const ar_data_t *right = parse_primary(ctx);
        if (!right) {
...
static const ar_data_t* parse_additive(ar_expression_context_t *ctx) {
    ...
    while (ctx->ref_expr[ctx->offset] == '+' || ctx->ref_expr[ctx->offset] == '-') {
        ...
        const ar_data_t *right = parse_multiplicative(ctx);
        if (!right) {
            // Don't destroy left - it's a borrowed reference
            return NULL;
        }
...
static const ar_data_t* parse_comparison(ar_expression_context_t *ctx) {
    ...
    const ar_data_t *right = parse_additive(ctx);
    if (!right) {
        // Don't destroy left - it's a borrowed reference
        return NULL;
    }
    ...
    ar_data_t *comparison_result = ar_data__create_integer(result ? 1 : 0);
...
const ar_data_t* ar_expression__evaluate(ar_expression_context_t *mut_ctx) {
    ...
    const ar_data_t *ref_result = parse_expression(mut_ctx);
...
ar_data_t* ar_expression__take_ownership(ar_expression_context_t *mut_ctx, const ar_data_t *ref_result) {
    ...
    ar_data_t *own_result = ar_list__remove(mut_ctx->own_results, ref_result);
