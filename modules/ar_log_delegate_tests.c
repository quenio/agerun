...
static void test_log_delegate__uses_sender_for_missing_agent_id(void);
static void test_log_delegate__uses_sender_for_mismatched_agent_id(void);
...
static ar_data_t* _create_log_message_without_agent_id(const char *ref_level, const char *ref_message) {
    ar_data_t *own_message = ar_data__create_map();
    if (!own_message) {
        return NULL;
    }
...
int main(void) {
    ...
    test_log_delegate__uses_sender_for_missing_agent_id();
    test_log_delegate__uses_sender_for_mismatched_agent_id();
...
static void test_log_delegate__uses_sender_for_missing_agent_id(void) {
    printf("  test_log_delegate__uses_sender_for_missing_agent_id...\n");

    ar_log_t *own_log = ar_log__create();
    ar_log_delegate_t *own_delegate = ar_log_delegate__create(own_log, "info");
    AR_ASSERT(own_delegate != NULL, "Setup: delegate created");

    ar_data_t *own_message = _create_log_message_without_agent_id("info", "hello");
    AR_ASSERT(own_message != NULL, "Setup: message created");

    ar_data_t *own_response = ar_log_delegate__handle_message(own_delegate, own_message, 10);
    AR_ASSERT(own_response != NULL, "Response should be returned");
    ...
    const char *ref_message = ar_event__get_message(ref_event);
    AR_ASSERT(ref_message != NULL, "Event should have message");
    AR_ASSERT(strstr(ref_message, "agent=10") != NULL, "Message should use sender id");
...
static void test_log_delegate__uses_sender_for_mismatched_agent_id(void) {
    printf("  test_log_delegate__uses_sender_for_mismatched_agent_id...\n");

    ar_log_t *own_log = ar_log__create();
    ar_log_delegate_t *own_delegate = ar_log_delegate__create(own_log, "info");
    AR_ASSERT(own_delegate != NULL, "Setup: delegate created");

    ar_data_t *own_message = _create_log_message("info", "hello", 42);
    AR_ASSERT(own_message != NULL, "Setup: message created");
    ...
    const char *ref_message = ar_event__get_message(ref_event);
    AR_ASSERT(ref_message != NULL, "Event should have message");
    AR_ASSERT(strstr(ref_message, "agent=7") != NULL, "Message should use sender id");
