...
int ar_semver__compare(const char *ref_v1, const char *ref_v2) {
    ...
    bool valid1 = ar_semver__parse(ref_v1, &maj1, &min1, &pat1);
    bool valid2 = ar_semver__parse(ref_v2, &maj2, &min2, &pat2);
...
bool ar_semver__are_compatible(const char *ref_v1, const char *ref_v2) {
    ...
    if (!ar_semver__parse(ref_v1, &maj1, NULL, NULL) ||
...
int ar_semver__find_latest_matching(const char **ref_versions, int count, const char *ref_pattern) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (ar_semver__matches_pattern(ref_versions[i], ref_pattern)) {
            // If this is the first match or it's newer than our current latest
            if (latest_idx == -1 || 
                ar_semver__compare(ref_versions[i], ref_versions[latest_idx]) > 0) {
                latest_idx = i;
            }
        }
    }
