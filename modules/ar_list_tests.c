...
static void test_create_destroy(void) {
    ...
    assert(ar_list__empty(own_list) == true);
...
static void test_add_last(void) {
    ...
    assert(ar_list__add_last(own_list, own_item1) == true);
    assert(ar_list__add_last(own_list, own_item2) == true);
    assert(ar_list__add_last(own_list, own_item3) == true);
    
    // Then the list should not be empty and have the correct count
    assert(ar_list__empty(own_list) == false);
    assert(ar_list__count(own_list) == 3);
    
    // When getting all items
    void **own_items = ar_list__items(own_list);
    size_t count = ar_list__count(own_list);
...
static void test_add_first(void) {
    ...
    assert(ar_list__add_first(own_list, own_item1) == true);
    assert(ar_list__add_first(own_list, own_item2) == true);
    assert(ar_list__add_first(own_list, own_item3) == true);
...
static void test_first_last(void) {
    ...
    assert(ar_list__last(own_list) == NULL);
    ...
    assert(ar_list__first(own_list) == own_item1);
    assert(ar_list__last(own_list) == own_item1);
    ...
    assert(ar_list__first(own_list) == own_item1);
    assert(ar_list__last(own_list) == own_item3);
...
static void test_remove_first_last(void) {
    ...
    assert(ar_list__remove_first(own_list) == NULL);
    assert(ar_list__remove_last(own_list) == NULL);
...
static void test_stack_operations(void) {
    ...
    assert(ar_list__count(own_list) == 3);
    assert(ar_list__first(own_list) == own_item3);
    
    // When using remove_first (pop) operations
    void *ref_popped1 = ar_list__remove_first(own_list);
    void *ref_popped2 = ar_list__remove_first(own_list);
    void *ref_popped3 = ar_list__remove_first(own_list);
...
static void test_queue_operations(void) {
    ...
    void *ref_dequeued1 = ar_list__remove_first(own_list);
    void *ref_dequeued2 = ar_list__remove_first(own_list);
    void *ref_dequeued3 = ar_list__remove_first(own_list);
...
static void test_add_many(void) {
    ...
    for (int i = 0; i < TEST_COUNT; i++) {
        ...
        assert(ar_list__add_last(own_list, own_expected_items[i]) == true);
    }
    
    // Then the list should have the correct count
    assert(ar_list__count(own_list) == TEST_COUNT);
...
static void test_null_parameters(void) {
    ...
    assert(ar_list__count(NULL) == 0);
    assert(ar_list__empty(NULL) == true);
    assert(ar_list__add_last(NULL, NULL) == false);
    assert(ar_list__add_first(NULL, NULL) == false);
    assert(ar_list__first(NULL) == NULL);
    assert(ar_list__last(NULL) == NULL);
    assert(ar_list__remove_first(NULL) == NULL);
    ...
    void **own_items = ar_list__items(NULL);
    assert(own_items == NULL);
    
    // Cleanup
    ar_list__destroy(own_list);
    ar_list__destroy(NULL);  // Should not crash
...
static void test_remove(void) {
    ...
    assert(ar_list__remove(own_list, NULL) == NULL);
    ...
    assert(ar_list__add_last(own_list, own_item2) == true);
    assert(ar_list__add_last(own_list, own_item3) == true);
    assert(ar_list__add_last(own_list, own_item4) == true);
    assert(ar_list__add_last(own_list, own_item5) == true);
    assert(ar_list__count(own_list) == 5);
    
    // When removing an item from the middle
    // Then it should return the item and update the list
    void *removed_item = ar_list__remove(own_list, own_item3);
    assert(removed_item == own_item3);
    assert(ar_list__count(own_list) == 4);
    ...
    // When removing the first item
    // Then it should return the item and update the list
    void *removed_item2 = ar_list__remove(own_list, own_item1);
    assert(removed_item2 == own_item1);
    assert(ar_list__count(own_list) == 3);
    
    // And the list should be updated correctly
    own_items = ar_list__items(own_list);
    assert(own_items != NULL);
    ...
    // When removing the last item
    // Then it should return the item and update the list
    void *removed_item3 = ar_list__remove(own_list, own_item5);
    ...
    assert(ar_list__remove(own_list, own_non_existent) == NULL);
    assert(ar_list__count(own_list) == 2);
    ...
    void *removed_item4 = ar_list__remove(own_list, own_item2);
    assert(removed_item4 == own_item2);
    assert(ar_list__count(own_list) == 2);
    
    // And the list should contain the remaining items
    assert(ar_list__first(own_list) == own_item4);
    assert(ar_list__last(own_list) == own_item2);
