...
const ar_tail_instruction_evaluator_t = struct {
    ref_log: ?*c.ar_log_t,
    ref_expr_evaluator: ?*c.ar_expression_evaluator_t,
};

pub export fn ar_tail_instruction_evaluator__create(
    ref_log: ?*c.ar_log_t,
    ref_expr_evaluator: ?*c.ar_expression_evaluator_t
) ?*ar_tail_instruction_evaluator_t {
...
pub export fn ar_tail_instruction_evaluator__destroy(
    own_evaluator: ?*ar_tail_instruction_evaluator_t
) void {
...
pub export fn ar_tail_instruction_evaluator__evaluate(
    ...
    if (c.ar_instruction_ast__get_type(ref_ast) != c.AR_INSTRUCTION_AST_TYPE__TAIL) {
        return false;
    }
