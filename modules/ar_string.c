...
char* ar_string__trim(char *mut_str) {
    ...
    while (ar_string__isspace(*ref_start)) ref_start++;
    ...
    while (mut_end > ref_start && ar_string__isspace(*mut_end)) mut_end--;
...
char* ar_string__path_parent(const char *ref_str, char separator) {
    ...
    size_t segments = ar_string__path_count(ref_str, separator);
    if (segments <= 1) {
