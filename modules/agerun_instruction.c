...
static bool parse_assignment(agent_t *agent, data_t *message, const char *instruction, int *pos) {
    ...
    *pos += 2; // Skip ':='
    ...
    expression_context_t *ctx = ar_expression_create_context(agent->memory, agent->context, message, instruction + *pos);
    if (!ctx) {
        free(path);
        return false;
    }
...
static bool parse_function_call(agent_t *agent, data_t *message, const char *instruction, int *pos, data_t **result) {
    ...
    *result = NULL;
    ...
    if (strcmp(function_name, "send") == 0) {
        ...
        expression_context_t *agent_id_ctx = ar_expression_create_context(agent->memory, agent->context, message, instruction + *pos);
        if (!agent_id_ctx) {
            return false;
        }
        ...
        *pos += ar_expression_offset(agent_id_ctx);
        ...
        expression_context_t *msg_ctx = ar_expression_create_context(agent->memory, agent->context, message, instruction + *pos);
        if (!msg_ctx) {
            ar_data_destroy(agent_id_data);
            return false;
        }
        ...
        *result = ar_data_create_integer(success ? 1 : 0);
        ...
        expression_context_t *cond_ctx = ar_expression_create_context(agent->memory, agent->context, message, instruction + *pos);
        if (!cond_ctx) {
            return false;
        }
        ...
        *pos += ar_expression_offset(cond_ctx);
        ...
        expression_context_t *true_ctx = ar_expression_create_context(agent->memory, agent->context, message, instruction + *pos);
        if (!true_ctx) {
            ar_data_destroy(cond_data);
            return false;
        }
        ...
        *pos += ar_expression_offset(true_ctx);
        ...
        expression_context_t *false_ctx = ar_expression_create_context(agent->memory, agent->context, message, instruction + *pos);
        if (!false_ctx) {
            ar_data_destroy(cond_data);
            ar_data_destroy(true_data);
            return false;
