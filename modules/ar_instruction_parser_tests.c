...
static void test_instruction_parser__parse_assignment(void) {
    ...
    assert(own_ast != NULL);
    assert(ar__instruction_ast__get_type(own_ast) == AR_INST__ASSIGNMENT);
...
static void test_instruction_parser__parse_destroy_agent(void) {
    ...
    // Then it should parse successfully
    assert(own_ast != NULL);
    assert(ar__instruction_ast__get_type(own_ast) == AR_INST__DESTROY_AGENT);
...
static void test_instruction_parser__parse_unknown(void) {
    ...
    // Then it should fail
    assert(own_ast == NULL);
    assert(ar__instruction_parser__get_error(own_parser) != NULL);
    assert(strstr(ar__instruction_parser__get_error(own_parser), "Unknown") != NULL);
...
static void test_parse_error_handling(void) {
    ...
    // Test 1: Invalid assignment operator
    {
        ...
        assert(error != NULL);
        assert(ar__instruction_parser__get_error_position(own_parser) > 0);
