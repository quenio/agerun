...
NULL list inputs, non-LIST list inputs, NULL appended values, and copy failures return integer `0`
per the central [SPEC.md sentinel contract](../SPEC.md#integer-0-sentinel-semantics). Appending an
ordinary integer `0` value to a valid list is normal list construction, not a sentinel case; this
includes missing path references that have already evaluated to integer `0`.
