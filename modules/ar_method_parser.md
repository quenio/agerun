...
- [x] Canonicalize top-level multi-line assignment and map-merge literals
...
13. **Literal Block Canonicalization**: Converts top-level assignment blocks such as `memory.items := [` and map merge blocks such as `memory += {` into a single logical assignment before instruction parsing
...
- The opener must be the full assignment right side: `memory.path := [`, `memory.path := {`, or `memory.path += {`
