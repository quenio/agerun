/**
 * @file ar_exit_instruction_evaluator.c
 * @brief Implementation of the exit instruction evaluator module
 */
...
struct ar_exit_instruction_evaluator_s {
...
static void _log_error(ar_exit_instruction_evaluator_t *mut_evaluator, const char *message) {
...
ar_exit_instruction_evaluator_t* ar_exit_instruction_evaluator__create(
    ...
    ar_exit_instruction_evaluator_t *own_evaluator = AR__HEAP__MALLOC(
        sizeof(ar_exit_instruction_evaluator_t), 
...
bool ar_exit_instruction_evaluator__evaluate(
    ...
    // Validate AST type
    if (ar_instruction_ast__get_type(ref_ast) != AR_INSTRUCTION_AST_TYPE__EXIT) {
        return false;
    }
    ...
    ar_data_t *agent_id_result = ar_expression_evaluator__evaluate(mut_expr_evaluator, ref_frame, ref_agent_id_ast);
