...
static void test_shell__process_input_stream_lists_active_agents_with_method_versions(void);
...
int main(void) {
    ...
    test_shell__process_input_stream_lists_active_agents_with_method_versions();
...
static void test_shell__process_input_stream_lists_active_agents_with_method_versions(void) {
    ...
    char ref_expected_shell_line[128];
    char ref_expected_echo_line[128];
    ...
    printf("  test_shell__process_input_stream_lists_active_agents_with_method_versions...\n");
    ...
    AR_ASSERT(fputs("memory.echo_id := spawn(\"echo\", \"1.0.0\", context)\nagents\n", own_input_stream) != EOF,
              "Input stream should accept spawn and agent-list shell lines");
    ...
    AR_ASSERT(processed_count == 2,
              "Shell delegate should process both the spawn and agents shell lines before EOF");

    echo_agent_id = ar_data__get_map_integer(ar_shell_session__get_memory(ref_session), "echo_id");
    AR_ASSERT(echo_agent_id > 0, "Assigned spawn should store echo_id for the agent-list command");
    ...
    AR_ASSERT(output_bytes_read > 0,
              "Shell output should contain handoff acknowledgements and the active-agent listing");
    AR_ASSERT(output_read_errno == 0, "Shell output read should not leave errno set on success");
    ref_output[output_bytes_read] = '\0';

    snprintf(ref_expected_shell_line, sizeof(ref_expected_shell_line),
             "agent id=%lld method=shell version=1.0.0\n", (long long)shell_agent_id);
    snprintf(ref_expected_echo_line, sizeof(ref_expected_echo_line),
             "agent id=%lld method=echo version=1.0.0\n", (long long)echo_agent_id);
    AR_ASSERT(strstr(ref_output, ref_expected_shell_line) != NULL,
              "Agent-list command should include the active shell receiving agent with method version");
    AR_ASSERT(strstr(ref_output, ref_expected_echo_line) != NULL,
              "Agent-list command should include spawned runtime agents with method versions");
