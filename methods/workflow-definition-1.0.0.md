...
If both schema validation and the startup dependency probe fail, schema validation remains
authoritative and the method reports `invalid_definition_schema`.
...
memory.dependency_failed_value := if(memory.dependency_status = "ready", 0, 1)
memory.startup_dependency_failed_flag := memory.schema_ok * memory.dependency_failed_value
memory.error_reason := if(message.action = "definition_file", "invalid_definition_schema", memory.error_reason)
memory.error_reason := if(memory.startup_dependency_failed_flag = 1, "startup_dependency_unavailable", memory.error_reason)
memory.error_category := if(memory.startup_dependency_failed_flag = 1, "runtime_unavailable", "")
