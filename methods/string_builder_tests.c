...
static void test_string_builder_parse_build(void) {
    ...
    ar__data__set_map_string(own_message, "template", "user={username}, role={role}");
    ar__data__set_map_string(own_message, "input", "user=alice, role=admin");
    ar__data__set_map_string(own_message, "output_template", "Welcome {username}! Your role is: {role}");
    ar__data__set_map_integer(own_message, "sender", 999);
    ...
    const data_t *parsed = ar__data__get_map_data(agent_memory, "parsed");
    if (parsed == NULL) {
        ...
        assert(ar__data__get_type(parsed) == DATA_MAP);
        ...
        // Try "username"
        const data_t *test_username = ar__data__get_map_data(parsed, "username");
        if (test_username) {
            printf("    username: %s\n", ar__data__get_string(test_username));
        }
        
        // Try "user"
        const data_t *test_user = ar__data__get_map_data(parsed, "user");
        if (test_user) {
            printf("    user: %s\n", ar__data__get_string(test_user));
        }
        
        // Try "role"
        const data_t *test_role = ar__data__get_map_data(parsed, "role");
        if (test_role) {
            printf("    role: %s\n", ar__data__get_string(test_role));
        }
        
        // Check that parsed contains username and role
        const data_t *username = ar__data__get_map_data(parsed, "username");
        if (username != NULL) {
            assert(ar__data__get_type(username) == DATA_STRING);
            assert(strcmp(ar__data__get_string(username), "alice") == 0);
            printf("  - Extracted username: %s\n", ar__data__get_string(username));
        } else {
            printf("  - WARNING: 'username' key not found in parsed map\n");
        }
        
        const data_t *role = ar__data__get_map_data(parsed, "role");
        if (role != NULL) {
            assert(ar__data__get_type(role) == DATA_STRING);
            assert(strcmp(ar__data__get_string(role), "admin") == 0);
            printf("  - Extracted role: %s\n", ar__data__get_string(role));
        } else {
    ...
    if (result == NULL) {
        ...
        assert(strcmp(ar__data__get_string(result), "Welcome alice! Your role is: admin") == 0);
    }
...
__attribute__((unused)) static void test_string_builder_parse_failure(void) {
    ...
    ar__data__set_map_string(own_message, "template", "name={name}, age={age}");
    ar__data__set_map_string(own_message, "input", "user=bob, role=user");
    ar__data__set_map_string(own_message, "output_template", "Hello {name}, you are {age} years old");
    ar__data__set_map_integer(own_message, "sender", 999);
    ...
    if (result == NULL) {
        ...
        const char *result_str = ar__data__get_string(result);
