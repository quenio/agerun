...
| Core language docs treat function calls as instructions, not expressions, and nested calls are forbidden, but some parser-module docs still advertise nested call support. | `AGENTS.md`, `kb/agerun-method-language-nesting-constraint.md`, `modules/ar_send_instruction_parser.md`, `modules/ar_condition_instruction_parser.md`, `modules/README.md`, disabled parser tests | The core language rule conflicts with composability for pure value-producing calls, but protects expression purity for effectful operations such as `send(...)`, `spawn(...)`, and `append(...)`. The stale parser-module wording also violates Single Source of Semantics because it can be read as end-to-end language support that the expression parser and disabled tests reject. | Treat pure call composition as the primary gap, keep side-effectful operations sequenced as instructions, and fix stale parser-module docs so they stop overstating nested-call support. |
...
| F1 | Built-in calls | Pure built-in calls are not composable as expressions. Calls are accepted only as top-level function instructions, with optional result assignment. | Function results are stored through instruction-specific result assignment, not by normal expression assignment. Effectful built-ins are correctly kept out of expressions if expression purity is a hard rule. | `SPEC.md` separates `<function-instruction>` from `<expression>`. `ar_expression_ast_t` has no call node, while `ar_instruction_ast_t` has per-call instruction types. `AGENTS.md` and `kb/agerun-method-language-nesting-constraint.md` say function calls are not expressions. `ar_expression_tests.c` and disabled parser tests state function calls in expressions are not supported, while `modules/ar_send_instruction_parser.md`, `modules/ar_condition_instruction_parser.md`, and `modules/README.md` still contain stale nested-call wording. | High |
...
| F9 | Documentation consistency | Documentation documents real constraints but not under a single principle. Some docs are stale or contradict each other. | Stale docs can make semantics appear less orthogonal than the implementation or hide actual couplings. Conflicting parser-module docs also make unsupported nested calls look implemented when they are not. | `AGENTS.md` and `kb/agerun-method-language-nesting-constraint.md` document non-expression calls. `modules/ar_send_instruction_parser.md`, `modules/ar_condition_instruction_parser.md`, and `modules/README.md` still advertise nested call support. `kb/agerun-language-constraint-workarounds.md` contradicts current lazy `if(...)`. | Medium |
...
Documentation is not fully aligned on this restriction. Core language references and tests treat
function calls as non-expression instructions, while `modules/ar_send_instruction_parser.md`,
`modules/ar_condition_instruction_parser.md`, and the parser summary in `modules/README.md` still
describe nested function-call support. Those parser-module docs reflect argument-splitting behavior
or stale expectations, not the current end-to-end method language contract, so they should be
treated as stale documentation rather than evidence that nested calls are currently supported.
