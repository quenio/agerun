...
entries to use statement-like `identifier := expression` syntax. One-line list/map expressions keep
their existing comma and `identifier: expression` grammar.
...
syntax and are canonicalized into the existing one-line map expression form internally. This keeps
AgeRun line-oriented and avoids promoting multiline literal parsing into `ar_expression_parser`.
...
multiline map entries use `identifier := expression`, and the next recommended follow-up is now the
sentinel semantics plan.
