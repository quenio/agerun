...
- [x] Executable no-persistence option: Added `agerun --no-persistence` plus `make run-exec NO_PERSISTENCE=1` so one-off runs skip loading and saving persisted methodology/agency state, keep existing persisted files untouched, and still allow fresh-start boot overrides; expanded executable coverage with no-persistence startup/save/override tests and updated executable-facing docs/specs to describe the new operator contract (Completed 2026-04-22)
- [x] Boot-safe executable echo override: Added `boot-echo-1.0.0.method`, `boot-echo-1.0.0.md`, and `methods/boot_echo_tests.c` so executable boot overrides can demonstrate `echo` without crashing on the raw `"__boot__"` startup string; updated executable docs/examples/tests to use `boot-echo-1.0.0` and expanded the runtime inventory from 14 to 15 methods (Completed 2026-04-20)
- [x] Executable complete() default model path fallback: Updated local completion path resolution in both the C and Zig runtimes so the default phi-3 model is found when `make run-exec` launches the executable from `bin/run-exec`; added `modules/ar_local_completion_path_tests.c` to verify the repo-relative fallback path from the run directory and confirmed fresh executable startup reaches the workflow completion path instead of failing on a missing model file (Completed 2026-04-18)
- [x] Workflow Coordinator Bootstrap Demo: Added `workflow-definition-1.0.0.method`, `workflow-coordinator-1.0.0.method`, `workflow-item-1.0.0.method`, and `workflow-reporter-1.0.0.method` with method documentation and tests; updated `bootstrap-1.0.0.method` and executable coverage to launch the workflow demo path, report startup dependency failures clearly when local `complete(...)` is unavailable, and validate the 14-method runtime inventory; synchronized workflow contracts/spec artifacts with the implemented behavior (Completed 2026-04-18)
- [x] Chat Session Method and Bootstrap Demo: Added `chat-session-1.0.0.method` as a stateful session-backend example with documentation and tests; updated `bootstrap-1.0.0.method` to spawn the chat-session agent and drive a sample start/message/summary flow; updated executable tests for the new 9-method inventory and bootstrap behavior (Completed 2026-04-10)
- [x] Checkpoint Tracking Removal from Commands: Removed checkpoint/gate tracking from all 30+ commands; replaced with session todo list tracking + step-verifier verification; updated KB articles (checkpoint-tracking-verification-separation.md, session-todo-list-tracking-pattern.md, sub-agent-verification-pattern.md) to reflect new agreement; all commands now use simplified workflow pattern without checkpoint scripts (Completed 2025-11-09)
- [x] Step Verification Enforcement: Added STEP VERIFICATION ENFORCEMENT sections and MANDATORY: Initialize All Todo Items sections to all 31 commands; updated CHECKPOINT WORKFLOW ENFORCEMENT to clarify verification separation; fixed broken KB link in .claude/step-verifier.md; renamed 4 Python scripts to use underscores per PEP 8; all commands now follow consistent step-verifier pattern with interleaved step/verification todo items initialized at workflow start (Completed 2025-11-09)
- [x] Documentation Validation Fix (Session 2025-10-18): Fixed 12 documentation validation errors across 4 files (.opencode/command/ar/{create,execute,review}-plan.md and kb/review-plan-command-improvements.md) by adding EXAMPLE tags to non-existent function and type references (ar_foo__*, ar_file_delegate_t); applied batch_fix_docs.py with dry-run verification; all validation errors resolved per validated-documentation-examples.md pattern; make check-docs passes (Completed 2025-10-18)
...
- [x] Phase 5 - Checkpoint Infrastructure Refactoring: Consolidated wrapper scripts into implementations; removed Makefile targets; net 175 lines reduction - 2025-10-18
...
- [x] **TDD Cycle 6.5**: Add message queue infrastructure to ar_delegation (PREREQUISITE for Cycle 7) - ✅ **COMPLETED (100%)** (Completed 2025-10-13)
  ...
  - [x] **Iterations 1-6**: Delegate message queue implementation (Completed 2025-10-13)
    - Implemented ar_delegate__send(), ar_delegate__has_messages(), ar_delegate__take_message()
...
- [x] **TDD Cycle 7 - Fixture Infrastructure (Cycle 0 COMPLETE)**: Update evaluators to route messages via delegation
  - [x] **Plan Created**: See `plans/message_routing_via_delegation_plan.md` for complete iteration breakdown
  - [x] **Fixture Implementation Complete (Iterations 0.1-0.8)**: All 9 iterations completed with strict RED-GREEN-REFACTOR discipline
    - [x] Create ar_send_evaluator_fixture module (iterations 0.1-0.8) - 9 tests passing, zero memory leaks
    - [x] Refactor 7 existing tests to use fixture helpers (iteration 0.8 final step) - all tests passing
    - [x] Total: 16 tests (9 fixture + 7 refactored) passing, zero memory leaks, zero memory allocations leaked
...
- [ ] **TDD Cycle 26**: Security testing and hardening
...
- [x] Complete CHANGELOG.md compaction from 1274 to 531 lines (58% reduction); achieved KB target of 40-50% for historical records; created scripts/compact_changelog_maximal.py with enforced gates; preserved all 157 metrics and chronological order (Completed 2025-10-08)
  ...
  - [x] Session 3: Intelligent manual compaction from 533 to 129 lines (75% line reduction, 86% file size reduction); combined 200+ repetitive entries into coherent summaries; exceeded KB target (40-50%) with human semantic analysis; preserved all 157+ metrics, dates, and milestones (Completed 2025-10-08)
  - [x] Target: 40-50% overall reduction while preserving all metrics and dates (EXCEEDED: 86% file size reduction achieved)
  - [x] Method: Manual rewriting and semantic combination of related entries across different dates
